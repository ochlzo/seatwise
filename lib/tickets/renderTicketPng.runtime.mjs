...
function resolveTicketRenderFontFamily(family) {
  const trimmedFamily = typeof family === "string" && family.trim() ? family.trim() : "Merriweather";
...
export async function renderTicketPngRuntime({ template, fields, qrDataUrls }) {
  ...
  for (const node of template.nodes) {
    ...
    if (node.kind === "qr") {
      const qrDataUrl = qrDataUrls?.[String(Math.max(Math.round(node.size), 1))];
      if (!qrDataUrl) {
        continue;
      }
