...
export function LoginForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isForgotPassword) {
      try {
        ...
        if (error.code === 'auth/user-not-found') {
          ...
          setValidationError("Failed to send reset email. Please try again later.")
        }
      }
      return
    }
    ...
    try {
      ...
      if (isGoogleSetup) {
        ...
        user = await signUpWithEmail(email, password, username, firstName, lastName)
      } else {
  ...
  return (
    <div className={cn("flex flex-col gap-6", (isForgotPassword || isGoogleSetup) && "max-w-md mx-auto", className)} {...props}>
      <style>{`
        @keyframes fadeIn {
          from { opacity: 0; }
          to { opacity: 1; }
      ...
      <Card className="overflow-hidden p-0">
        <CardContent
          key={isForgotPassword ? "forgot" : isGoogleSetup ? "setup" : isSignUp ? "signup" : "login"}
          className={cn(
            ...
            <FieldGroup>
              <div className="flex flex-col items-center gap-2 text-center">
                <h1 className="text-2xl font-bold">
                  {isForgotPassword ? "Reset Password" : isGoogleSetup ? "Complete Profile" : isSignUp ? "Create an account" : "Welcome back"}
                </h1>
                ...
                <>
                  {!resetEmailSent ? (
                    ...
                    <div className="text-center my-4 flex flex-col items-center">
                      <img src="/check.png" alt="Success" className="w-12 h-12 mb-2" />
                      <p className="text-green-600 font-medium">Password reset email sent! Check your inbox.</p>
                      <p className="text-sm text-muted-foreground mt-1">If you don't see it, check your spam folder.</p>
                    </div>
                  )}
                  <div className="flex flex-col gap-2">
                    {!resetEmailSent && <Button type="submit">Send Reset Link</Button>}
                    <Button
                      variant="ghost"
                      type="button"
                      onClick={() => {
